...
const encrypt = () => {
  ...
  try {
    ...
    const ivBytes = iv.value ? CryptoJS.enc.Utf8.parse(iv.value.padEnd(8, '0').substring(0, 8)) : undefined
    let encrypted
    switch (mode.value) {
      case 'CBC':
        encrypted = CryptoJS.DES.encrypt(inputText.value, keyBytes, {
          iv: ivBytes,
...
const decrypt = () => {
  ...
  try {
    ...
    switch (mode.value) {
      case 'CBC':
        decrypted = CryptoJS.DES.decrypt(inputText.value, keyBytes, {
          iv: ivBytes,
          mode: CryptoJS.mode.CBC,
          padding: getPadding(padding.value)
        })
...
const jsCode = computed(() => {
  return `const CryptoJS = require('crypto-js');\n\nconst key = '${key.value}';\nconst iv = '${iv.value}';\nconst text = '${inputText.value}';\n\n// 加密\nconst encrypted = CryptoJS.DES.encrypt(text, CryptoJS.enc.Utf8.parse(key), {\n  ${mode.value !== 'ECB' ? "iv: CryptoJS.enc.Utf8.parse(iv.padEnd(8, '0').substring(0, 8))," : ''}\n  mode: CryptoJS.mode.${mode.value},\n  padding: CryptoJS.pad.${padding.value}\n});\nconsole.log('加密结果:', encrypted.toString());\n\n// 解密\nconst decrypted = CryptoJS.DES.decrypt(encrypted.toString(), CryptoJS.enc.Utf8.parse(key), {\n  ${mode.value !== 'ECB' ? "iv: CryptoJS.enc.Utf8.parse(iv.padEnd(8, '0').substring(0, 8))," : ''}\n  mode: CryptoJS.mode.${mode.value},\n  padding: CryptoJS.pad.${padding.value}\n});\nconsole.log('解密结果:', decrypted.toString(CryptoJS.enc.Utf8));`
})

const pythonCode = computed(() => {
  return `from Crypto.Cipher import DES\nfrom Crypto.Util.Padding import pad, unpad\nimport base64\n\nkey = b'${key.value.padEnd(8, '0').substring(0, 8)}'\n${mode.value !== 'ECB' ? `iv = b'${iv.value.padEnd(8, '0').substring(0, 8)}'` : ''}\ntext = b'${inputText.value}'\n\n# 加密\ncipher = DES.new(key, DES.MODE_${mode.value}${mode.value !== 'ECB' ? ', iv=iv' : ''})\nct_bytes = cipher.encrypt(pad(text, 8))\nenc = base64.b64encode(ct_bytes).decode()\nprint('加密结果:', enc)\n\n# 解密\ncipher2 = DES.new(key, DES.MODE_${mode.value}${mode.value !== 'ECB' ? ', iv=iv' : ''})\npt = unpad(cipher2.decrypt(base64.b64decode(enc)), 8)\nprint('解密结果:', pt.decode())`
})
</script>
